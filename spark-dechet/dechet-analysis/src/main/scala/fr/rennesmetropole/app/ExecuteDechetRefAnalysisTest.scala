...
object ExecuteDechetRefAnalysisTest{
  def main(args: Array[String]):  Unit = {
    ...
    try {
      ...
      log("LATEST PRODUCTEUR REF SHOW")
      ...
      log("LATEST BAC REF SHOW")
      df_lastestBac.show()
      val t1 = System.nanoTime()
      log("Elapsed time before join: " + (t1 - t0)/1000000000 + "s (" + (t1 - t0) + " ns)")
      val join = df_lastestProducteur.join(df_lastestBac,Seq("code_producteur"),"inner")
      log("Jointure prod x bac")
      join.show()
      val t2 = System.nanoTime()
      log("Elapsed time for join: " + (t2 - t1) / 1000000000 + "s (" + (t2 - t1) + " ns)")
      log("Total elapsed time: " + (t2 - t0)/1000000000 + "s (" + (t2 - t0) + " ns)")
    } catch {
      case e: Throwable => {
