...
class ImportDechetTest {
  ...
  def readFromMinioTest(): Unit = {
    ...
    val nameEnv= "test"
    var dfInput = ImportDechet.readFromMinio(spark, DATE,nameEnv)
  ...
  @Test
  def ExecuteImportDecheTest(): Unit = {
    var DATE = java.time.LocalDate.now.toString
    val nameEnv= "test"
    var dfInput = ImportDechet.ExecuteImportDechet(spark, DATE,nameEnv)
